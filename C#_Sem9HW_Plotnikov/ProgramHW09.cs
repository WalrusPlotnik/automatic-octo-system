...
// функция обработки данных рекурсивное вычисление функции Аккермана
int AckermannFunction (int m, int n){
    if (m == 0){
        return n+1; // при m = 0 функция Аккермана определяется как n + 1
    }
    else if(m != 0 && n == 0){
        return AckermannFunction(m-1,1); // при m > 0, n = 0 функция Аккермана определяется как A(m-1, 1)
    }
    else if(m > 0 && n > 0){
        return AckermannFunction(m-1, AckermannFunction(m,n-1)); // при m > 0, n > 0 функция Аккермана определяется как A(m-1, А(m, n-1))
    }
...
// объявляем переменные входных данных через функцию ввода
int m = InputData("Введите число М: ");
int n = InputData("Введите число N: ");
// присваиваем эту переменную через функцию обработки данных (рекурсивное вычисление функции Аккермана)
int result = AckermannFunction(m,n); 
// вызываем функцию вывода, другие две функции она вызовет сама
ShowData("Значение функции Аккермана при ваших исходных M и N в составляет: ", result.ToString()); // с конвертацией в string
